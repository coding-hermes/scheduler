...
// Same shape as internal/scheduler/gap060_schema_test.go: the template is
// built once per test binary and each test gets a byte-level COPY in its own
// t.TempDir(), so tests stay fully isolated while paying one migration chain
// per binary instead of one per test (31 call sites in this package).
