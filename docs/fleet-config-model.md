...
```
FAIL — 58 violation(s) across 12 namespaces / 156 enabled lanes
```

by class: `family-floor 51`, `targets 4`, `cooldown 2`, `parity 1`. **How to read this honestly:** the checker is green on the caps, admission-mode, executor, workdir, adaptive-arming, board-present, coverage and board-vocabulary classes, and red on four others — the 51 `family-floor` violations are satellite lanes sitting on the 6 h default instead of their 43200 family pin, the 4 `targets` violations are the four `hermes-dagger-*` satellites whose primary project is disabled, and the two `cooldown` + one `parity` violations are the `hermes-canopy` / `coding-hermes-tools` 900 s rows from §4.1 and §5.
